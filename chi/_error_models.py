...
class ErrorModel(object):
    ...
    def __init__(self):
        ...
        self._normalised_log_likelihood = False
    ...
    def compute_normalised_log_likelihood(self, parameters, model_output, observations):
        r"""
        ...
        """
        pLL = self.compute_normalised_pointwise_ll(parameters, model_output, observations)
        return self._nansum(pLL)

    def compute_normalised_pointwise_ll(self, parameters, model_output, observations):
        r"""
    ...
    def set_normalised_log_likelihood(self, value):
        """
        Divide the log likelihood by the mean log observation before returning.

        :param value: A boolean.
        """
        self._normalised_log_likelihood = value
...
class ConstantAndMultiplicativeGaussianErrorModel(ErrorModel):
    ...
    def compute_log_likelihood(self, parameters, model_output, observations):
        ...
        if self._normalised_log_likelihood:
            return self.compute_normalised_log_likelihood(parameters, model_output, observations)
    ...
    def compute_normalised_pointwise_ll(self, parameters, model_output, observations):
        ...
        L = self._compute_pointwise_ll(parameters, model, obs)
...
class LogNormalErrorModel(ErrorModel):
    ...
    def compute_normalised_pointwise_ll(self, parameters, model_output, observations):
        r"""
            Returns the log-likelihood of model parameters, L(\psi, \sigma | x^{\text{obs}}), with units of 1/observations multiplied out. All quantitaties with units of [observations], including model_output, are divided by the observations. In this case,
...
class LogTransformedErrorModel(ErrorModel):
    """
        A transformation of any another error model, which takes the log of given observations and estimates before passing
        them to the underlying error model.
    """
    def __init__(self, error_model):
        super(LogTransformedErrorModel, self).__init__()
        ...
        if not isinstance(error_model, ErrorModel):
            raise ValueError(
                ...
                'chi.ErrorModel')
        if isinstance(error_model, LogTransformedErrorModel):
            raise ValueError(
                'The error model must not be an instance of a '
                'chi.LogTransformedErrorModel')

        self._error_model = error_model
        self.set_parameter_names()

    def compute_log_likelihood(self, parameters, model_output, observations):
        if np.any(model_output <= 0) or np.any(observations <= 0):
            return -np.inf

        return self._error_model.compute_log_likelihood(parameters, np.log(model_output), np.log(observations))
    ...
    def compute_pointwise_ll(self, parameters, model_output, observations):
        ...
        return self._error_model.compute_pointwise_ll(parameters, np.log(model_output), np.log(observations))

    def compute_normalised_log_likelihood(self, parameters, model_output, observations):
        if np.any(model_output <= 0) or np.any(observations <= 0):
            return -np.inf

        return self._error_model.compute_normalised_log_likelihood(
            parameters, np.log(model_output), np.log(observations))
    ...
    def compute_normalised_pointwise_ll(self, parameters, model_output, observations):
        ...
        return self._error_model.compute_normalised_pointwise_ll(
            parameters, np.log(model_output), np.log(observations))

    def compute_sensitivities(self, parameters, model_output, model_sensitivities, observations):
        if np.any(model_output <= 0) or np.any(observations <= 0):
            n_parameters = model_sensitivities.shape[1] + 1
            return -np.inf, np.full(n_parameters, np.inf)

        return self._error_model.compute_sensitivities(
            parameters, np.log(model_output), model_sensitivities, np.log(observations))
    ...
    def set_parameter_names(self, names=None):
        #Use underyling error model to set names
        self._error_model.set_parameter_names(names)

        #Retrieve the names and add a log-transform marker to them
        names = self._error_model.get_parameter_names()
        names = [(nam + " LogTrans") if "LogTrans" not in nam else nam for nam in names]
...
class ReducedErrorModel(object):
    ...
    def set_normalised_log_likelihood(self, value):
        ...
        """
        self._normalised_log_likelihood = value
        self._error_model._normalised_log_likelihood = value
