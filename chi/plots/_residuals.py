...
class ResidualPlot(plots.SingleFigure):
    ...
    def _add_predicted_versus_observed_scatter_plot(
        ...
        id_key, time_key_m, _, value_key = self._keys
        ...
        for index, _id in enumerate(ids):
            ...
            observations = temp[value_key].to_numpy()
    ...
    def _get_relevant_measurements(
            self, data, observable, individual, time_key):
        """
        Filters the observations for the relevant observable and ID. Also makes
        sure that there is a prediction for each measured time point.
        """
        # Get keys of measurement dataframe
        id_key_m, time_key_m, obs_key_m, _ = self._keys
        ...
        # Mask measurements for observable
        mask = measurements[obs_key_m] == observable
        measurements = measurements[mask]
